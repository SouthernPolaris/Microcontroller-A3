...
void adc_init(void) {
	...
	*ADC0_Port &= ~IOCON_DIGITAL_MODE;
	...
	//Enable ADC 2
	*(ADC0_Port+1) |= IOCON_ADC_PIN_FUNC_1; 
	*(ADC0_Port+1) &= ~IOCON_DIGITAL_MODE;
...
int adc_read(char channel) {
	...
	uint32_t data;
	LPC_ADC -> CR &= (~0xFFUL);
	LPC_ADC -> CR |= (1UL<<channel);
	
	LPC_ADC -> CR |= ADC_START; //Start conversion
	
	while( !(LPC_ADC->DR[channel] & (1UL<<31)) );//wait until the conversion completes
	LPC_ADC -> CR &= ~ADC_START;
	
	data = ((LPC_ADC->DR[channel] >> 4) ) & 0xFFF;
	return (data);
